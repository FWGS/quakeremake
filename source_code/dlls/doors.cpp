...
void CDoorTrigger :: Touch( CBaseEntity *pOther )
{
	if( !m_pDoor )
	{
		...
		SetThink( &CDoorTrigger::SUB_Remove );
		return; 
	}
...
void CBaseDoor::DoorTouch( CBaseEntity *pOther )
{
	...
	if (( pev->team & pOther->m_iItems ) != pev->team )
	{
		...
		else if (m_pOwner->pev->team & IT_KEY1 && m_pOwner->pev->team & IT_KEY2)
		{
			if (g_iWorldType == WORLDTYPE_PRESENT)
			{
				CenterPrint( pOther->pev, "You need the both silver and gold keycard" );
			}
			else if (g_iWorldType == WORLDTYPE_RUNIC)
			{
				CenterPrint( pOther->pev, "You need the both silver and gold runekey" );
			}
			else if (g_iWorldType == WORLDTYPE_MEDIEVAL)
			{
				CenterPrint( pOther->pev, "You need the both silver and gold key" );
			}
		}
