...
void ClientPutInServer( edict_t *pEntity )
{
	...
	if( !g_pGameRules->IsMultiplayer( ))
		g_flAutoSaveTime = gpGlobals->time + 1.0f;
}
...
void StartFrame( void )
{
	...
	if( g_flAutoSaveTime != 0.0f && g_flAutoSaveTime < gpGlobals->time )
	{
		...
		UTIL_FileBase ( STRING( gpWorld->pev->model ), mapname );
		SERVER_COMMAND( UTIL_VarArgs( "save %s\n", mapname ));
		g_flAutoSaveTime = 0.0f;
	}

	if ( g_psv_quakehulls != NULL && g_psv_quakehulls->value != 1.0f )
	{
		// hull-select properly method for Quake
		CVAR_SET_FLOAT( "sv_quakehulls", 1.0f );
	}
...
void ClientObituary( entvars_t *pevVictim, entvars_t *pevAttacker )
{
	...
	if (FClassnameIs( pevVictim, "player"))
	{
		...
		{
			g_engfuncs.pfnServerPrint( UTIL_VarArgs( "%s was telefragged by %s\n", STRING( pevVictim->netname ), STRING( VARS( pevAttacker->owner )->netname )));
		...
		{
			g_engfuncs.pfnServerPrint( UTIL_VarArgs( "Satan's power deflects %s's telefrag\n", STRING( pevVictim->netname )));
		...
		if (FClassnameIs( pevAttacker, "player"))
		{
			if (pevVictim == pevAttacker)
			{
				...
				if (pVictim->m_iWeapon == IT_LIGHTNING && pevVictim->waterlevel > 1)
				{
					g_engfuncs.pfnServerPrint (" discharges into the water.\n");
					return;
				}
				if (pVictim->m_iWeapon == IT_GRENADE_LAUNCHER)
					g_engfuncs.pfnServerPrint (" tries to put the pin back in\n");
				else if (rnum)
					g_engfuncs.pfnServerPrint (" becomes bored with life\n");
				else
					g_engfuncs.pfnServerPrint (" checks if his weapon is loaded\n");
				return;
			}
			else
			{
				char deathstring[128], deathstring2[128];
				...
				if (rnum == IT_AXE)
				{
					strcpy( deathstring, " was ax-murdered by " );
					strcpy( deathstring2, "\n" );
				}
				if (rnum == IT_SHOTGUN)
				{
					strcpy( deathstring, " chewed on ");
					strcpy( deathstring2, "'s boomstick\n");
				}
				if (rnum == IT_SUPER_SHOTGUN)
				{
					strcpy( deathstring, " ate 2 loads of ");
					strcpy( deathstring2, "'s buckshot\n");
				}
				if (rnum == IT_NAILGUN)
				{
					strcpy( deathstring, " was nailed by ");
					strcpy( deathstring2, "\n");
				}
				if (rnum == IT_SUPER_NAILGUN)
				{
					strcpy( deathstring, " was punctured by ");
					strcpy( deathstring2, "\n");
				}
				if (rnum == IT_GRENADE_LAUNCHER)
				{
					strcpy( deathstring, " eats ");
					strcpy( deathstring2, "'s pineapple\n");

					if (pevVictim->health < -40)
					{
						strcpy( deathstring, " was gibbed by ");
						strcpy( deathstring2, "'s grenade\n");
					}
				}
				if (rnum == IT_ROCKET_LAUNCHER)
				{
					strcpy( deathstring, " rides ");
					strcpy( deathstring2, "'s rocket\n");
				...
				{
					strcpy( deathstring, " accepts " );

					if (pevAttacker->waterlevel > 1)
						strcpy( deathstring2, "'s discharge\n" );
					else
				...
				g_engfuncs.pfnServerPrint (deathstring2);
			}
			return;
		}
		else
		{
			...
			if (rnum == CONTENTS_WATER)
			{
				if (RANDOM_FLOAT(0,1) < 0.5)
					g_engfuncs.pfnServerPrint (" sleeps with the fishes\n");
				else
					g_engfuncs.pfnServerPrint (" sucks it down\n");
				return;
			}
			else if (rnum == CONTENTS_SLIME)
			{
				if (RANDOM_FLOAT(0,1) < 0.5)
					g_engfuncs.pfnServerPrint (" gulped a load of slime\n");
				else
					g_engfuncs.pfnServerPrint (" can't exist on slime alone\n");
				return;
			}
			else if (rnum == CONTENTS_LAVA)
			{
				...
				{
					g_engfuncs.pfnServerPrint (" burst into flames\n");
					return;
				}
				if (RANDOM_FLOAT(0,1) < 0.5)
					g_engfuncs.pfnServerPrint (" turned into hot slag\n");
				else
					g_engfuncs.pfnServerPrint (" visits the Volcano God\n");
				return;
			}

			if (pevAttacker->flags & FL_MONSTER)
			{
				if (FClassnameIs( pevAttacker, "monster_army"))
					g_engfuncs.pfnServerPrint (" was shot by a Grunt\n");
				if (FClassnameIs( pevAttacker, "monster_demon1"))
					g_engfuncs.pfnServerPrint (" was eviscerated by a Fiend\n");
				if (FClassnameIs( pevAttacker, "monster_dog"))
					g_engfuncs.pfnServerPrint (" was mauled by a Rottweiler\n");
				if (FClassnameIs( pevAttacker, "monster_dragon"))
					g_engfuncs.pfnServerPrint (" was fried by a Dragon\n");
				if (FClassnameIs( pevAttacker, "monster_enforcer"))
					g_engfuncs.pfnServerPrint (" was blasted by an Enforcer\n");
				if (FClassnameIs( pevAttacker, "monster_fish"))
					g_engfuncs.pfnServerPrint (" was fed to the Rotfish\n");
				if (FClassnameIs( pevAttacker, "monster_hell_knight"))
					g_engfuncs.pfnServerPrint (" was slain by a Death Knight\n");
				if (FClassnameIs( pevAttacker, "monster_knight"))
					g_engfuncs.pfnServerPrint (" was slashed by a Knight\n");
				if (FClassnameIs( pevAttacker, "monster_ogre"))
					g_engfuncs.pfnServerPrint (" was destroyed by an Ogre\n");
				if (FClassnameIs( pevAttacker, "monster_oldone"))
					g_engfuncs.pfnServerPrint (" became one with Shub-Niggurath\n");
				if (FClassnameIs( pevAttacker, "monster_shalrath"))
					g_engfuncs.pfnServerPrint (" was exploded by a Vore\n");
				if (FClassnameIs( pevAttacker, "monster_shambler"))
					g_engfuncs.pfnServerPrint (" was smashed by a Shambler\n");
				if (FClassnameIs( pevAttacker, "monster_tarbaby"))
					g_engfuncs.pfnServerPrint (" was slimed by a Spawn\n");
				if (FClassnameIs( pevAttacker, "monster_vomit"))
					g_engfuncs.pfnServerPrint (" was vomited on by a Vomitus\n");
