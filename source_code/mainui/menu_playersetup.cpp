...
static void UI_PlayerSetup_UpdateConfig( void )
{
	...
	if( !stricmp( name, "player" ))
	{
		...
		strcpy( newImage, "models/player.bmp" );
	}
	else
	{
		sprintf( path, "models/player/%s/%s.mdl", name, name );
	...
	if( !ui_showmodels->value )
	{
		if( stricmp( lastImage, newImage ))
		{
			...
			if( !stricmp( name, "player" ))
			{
				sprintf( lastImage, "models/player.bmp" );
...
static void UI_PlayerSetup_Init( void )
{
	...
	uiPlayerSetup.ent->origin[0] = uiPlayerSetup.ent->curstate.origin[0] = 40.0f / tan( DEG2RAD( uiPlayerSetup.rvp.fov_y / 2.0f ));
