...
Vector CGib :: VelocityForDamage( float flDamage )
{
	...
	vecVelocity.x = RANDOM_FLOAT( 0, 100 );
	vecVelocity.y = RANDOM_FLOAT( 0, 100 );
...
void CBaseEntity::TraceAttack(entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType)
{
	...
	if ( pev->takedamage )
	{
		...
		else
		{
			...
			UTIL_ParticleEffect( vecOrigin, g_vecAttackDir, 73, flDamage * 2 );
		}
	}
...
================
*/
void CBaseEntity :: FireBullets( int iShots, Vector vecDir, Vector vecSpread )
{
	TraceResult trace;
